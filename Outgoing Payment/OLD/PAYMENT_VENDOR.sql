...
CASE WHEN VPM2."InvType" IN (30,46) THEN (Case when OVPM."DocCurr"='KES' 
...
left join 		
	...
	,Case WHEN "DocCur"='INR' THEN "DocTotal" ELSE "DocTotalFC" End AS "DocTotal","WTSum","VatSum",	
	 "TotalExpns","NumAtCard" from opch	
	 union all	
	 select "DocNum","DocEntry","CardCode","CardName","DocDate","Series","ObjType"
	 ,CASE WHEN "DocCur"='INR' THEN (-1)*"DocTotal" ELSE (-1)*"DocTotalFC" END AS "DocTotal","WTSum","VatSum",	
	 "TotalExpns","NumAtCard" from orpc	
	 union all	
	 select "Number" "DocNum","TransId" "DocEntry",NULL "CardCode",NULL "CardName","RefDate","Series","TransType",
	 null "DocTotal",null "WTSum",	
