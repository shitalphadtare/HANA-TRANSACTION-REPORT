...
POR."DocEntry", I."ExpnsName" AS "IEX", CASE WHEN POR."DocCur" = 'INR' THEN CGSTBASESUM ELSE 
CGSTBASESUMFRG END AS "CGSTBASESUM",
 CASE WHEN POR."DocCur" = 'INR' THEN CGSTTAXSUM ELSE CGSTTAXSUMFRG END AS "CGSTTAXSUM", CGSTTAXRATE, 
 CASE WHEN POR."DocCur" = 'INR' THEN SGSTBASESUM ELSE SGSTBASESUMFRG END AS "SGSTBASESUM", SGSTRATE, 
 CASE WHEN POR."DocCur" = 'INR' THEN SGSTTAXSUM ELSE SGSTTAXSUMFRG END AS "SGSTTAXSUM", 
 CASE WHEN POR."DocCur" = 'INR' THEN IGSTBASESUM ELSE IGSTBASESUMFRGN END AS "IGSTBASESUM", 
 IGSTRATE, CASE WHEN POR."DocCur" = 'INR' THEN IGSTTAXSUM ELSE IGSTTAXSUMFRG END AS "IGSTTAXSUM" 
 ...
 LEFT OUTER JOIN (SELECT DISTINCT i4."DocEntry", ox."ExpnsCode", ox."ExpnsName", ox."SacCode",
  ...
  FROM POR4 i4 LEFT OUTER JOIN OEXD ox ON ox."ExpnsCode" = i4."ExpnsCode" WHERE "RelateType" IN 
  (2,3) AND "staType" = -100 GROUP BY ox."ExpnsCode", ox."ExpnsName", i4."DocEntry", ox."SacCode") 
  AS C ON POR."DocEntry" = c."DocEntry" 
  ...
  SUM(i4."BaseSum") AS "SGSTBASESUM", SUM(i4."BaseSumFrg") AS "SGSTBASESUMFRG" FROM POR4 i4 
  ...
  c."ExpnsCode" = s."ExpnsCode" AND s."DocEntry" = POR."DocEntry";